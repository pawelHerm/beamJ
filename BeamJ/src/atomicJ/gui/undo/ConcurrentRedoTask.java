...
public class ConcurrentRedoTask extends MonitoredSwingWorker<Void, Void> 
{
    ...
    public ConcurrentRedoTask(ResourceView<?, ?, ?> resourceManager, List<? extends ChannelResource<?, ?, ?>> resources, String type)
    {
    ...
    public ConcurrentRedoTask(ResourceView<?, ?, ?> resourceManager, MultiMap<ChannelResource<?, ?, ?>, String> map)
    {
    ...
    public void done()
    {
        ...
        if(cancelled)
        {
            JOptionPane.showMessageDialog(resourceManager.getAssociatedWindow(), "Redoing terminated", "AtomicJ", JOptionPane.INFORMATION_MESSAGE);
        }
        else
        {
            ...
            if(failures > 0)
            {
                JOptionPane.showMessageDialog(resourceManager.getAssociatedWindow(), "Errors occured during redoing of " + failures + " operations", "AtomicJ", JOptionPane.ERROR_MESSAGE);
            }
        } 	
