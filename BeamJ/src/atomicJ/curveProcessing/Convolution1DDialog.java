...
public class Convolution1DDialog extends BatchProcessingDialog<Convolution1DModel<?>, String> implements PropertyChangeListener, KernelChangeListener
{
    ...
    private final Kernel1DPanel panelKernelData = new Kernel1DPanel();

    private final TextFileChooser fileChooser = new TextFileChooser();

    public Convolution1DDialog(Window parent, String title, boolean temporary)
    {
    ...
    {
        fieldDivisor.addPropertyChangeListener(VALUE_EDITED, new PropertyChangeListener()
        {
            @Override
            public void propertyChange(PropertyChangeEvent evt)
            {
                Double valueNew = ((Number)evt.getNewValue()).doubleValue();
                getModel().setDivisor(valueNew);
            }
        });

        fieldOffset.addPropertyChangeListener(VALUE_EDITED, new PropertyChangeListener()
        {
            @Override
            public void propertyChange(PropertyChangeEvent evt)
            {
                Double valueNew = ((Number)evt.getNewValue()).doubleValue();
                getModel().setOffset(valueNew);
            }
        });
    }
    ...
    {	
        ...
        mainPanel.addComponent(panelNormalize, 1, 4, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.NONE, 1, 1);
    ...
    @Override
    protected void setModel(Convolution1DModel<?> modelNew)
    {
    ...
    protected void clearOldModel(Convolution1DModel<?> model)
    {
        super.clearOldModel();
    ...
    protected void pullModelParameters()
    {
        ...
        Convolution1DModel<?> model = getModel();
    ...
    public void propertyChange(PropertyChangeEvent evt)
    {
        ...
        else if(Convolution1DModel.DIVISOR.equals(property))
        {
            Double valueNew = ((Number)evt.getNewValue()).doubleValue();
        ...
        else if(Convolution1DModel.OFFSET.equals(property))
        {
            Double valueNew = ((Number)evt.getNewValue()).doubleValue();
        ...
        else if(Convolution1DModel.NORMALIZE.equals(property))
        {
