...
public final class InterceptlessQuadratic implements FittedLinearUnivariateFunction
{
    ...
    {
        return new InterceptlessQuadratic(this.b/h, this.b/(h*h));
    }
    ...
    private void computeSqrt(double[] yalues,double[] residualsSqrt)
    {
        int n = yalues.length - 1;
        for(int i = n;i>=0;i--)
        {
            double r = residual(i, yalues[i]);
            residualsSqrt[i] = r*r;
        }
    }
    ...
    public double trimmedSquares(double[] yPoints,double[] residualsSqrt,int c)
    {
        computeSqrt(yPoints,residualsSqrt);
        Selector.sortSmallest(residualsSqrt, c);
        double crit = 0;
        for(int i = 0;i<c;i++)
        {
